create table global_data.third_party_study_results
(
    ...
    constraint third_party_correlations_cause_variables_id_fk
        foreign key (cause_id) references global_data.global_variables (id),
    ...
    constraint third_party_correlations_effect_variables_id_fk
        foreign key (effect_id) references global_data.global_variables (id)
)
    comment 'Stores Calculated Correlation Coefficients' charset = utf8;

create index cause
    on global_data.third_party_study_results (cause_id);

create index effect
    on global_data.third_party_study_results (effect_id);
