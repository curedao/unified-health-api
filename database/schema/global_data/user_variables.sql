create table global_data.user_variables
(
    ...
    constraint user_variables_correlations_qm_score_fk
        foreign key (best_user_correlation_id) references global_data.user_study_results (id)
            on delete set null,
    ...
    constraint user_variables_last_unit_id_fk
        foreign key (last_unit_id) references global_data.units (id),
...
create index fk_variableSettings
    on global_data.user_variables (variable_id);

create index user_variables_analysis_started_at_index
    on global_data.user_variables (analysis_started_at);

create index user_variables_user_id_latest_tagged_measurement_time_index
    on global_data.user_variables (user_id);

create index variables_analysis_ended_at_index
    on global_data.user_variables (analysis_ended_at);
